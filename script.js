document.addEventListener("DOMContentLoaded", function ()
{
	...
	function loadNotes() {
		...
		for(const note of notes) {
			...
			noteElement.setAttribute("data-note-id", note.id.toString()); // stores note ID to its data structure
			noteElement.value = note.content // Sets the note ID as value
			noteElement.className = "note"; // Set CSS Class
			noteElement.style.backgroundColor = noteColor; // Sets note's background color using last selected note color
			noteContainer.appendChild(noteElement); //Appends to note container element as its child			
		}
	};
